-- Feed Sorting & Retention Fix
-- 1. New stories (last 24h) always appear first
-- 2. 5-day retention: Stories older than 5 days are removed from feed
-- 3. Read/skipped stories don't appear (they're in History)

-- ============================================
-- RPC: GET UNSEEN STORIES (NEW SORTING + RETENTION)
-- ============================================
...
AS $$
  ...
  WHERE
    ...
    -- Topic filter - include stories with NULL/empty topic_ids
    AND (
      p_topic_ids IS NULL
  ...
  ORDER BY
    -- NEW FIRST: Stories from last 24 hours come before older ones
    CASE WHEN s.published_at > NOW() - INTERVAL '24 hours' THEN 0 ELSE 1 END,
    -- Within each group, sort by published_at DESC
...
AS $$
  ...
  WHERE
    ...
    -- Topic filter with NULL/empty handling
    AND (
      p_topic_ids IS NULL
      OR s.topic_ids && p_topic_ids
      OR s.topic_ids IS NULL
...
-- Index to optimize the new sorting (new-first, then by date)
CREATE INDEX IF NOT EXISTS idx_stories_feed_sort
